...
void GetLed(void) {
    char packet[MAX_PACKET_LENGTH] = {0};

    PacketBuilder(packet, "LED", ALL, GET);

    HAL_UART_Transmit(&huart4, (uint8_t*)packet, strlen(packet), MAX_TIMEOUT_MS);

    char data[MAX_PACKET_LENGTH] = {0};
    if(HAL_UART_Receive(&huart4, (uint8_t*)data, MAX_PACKET_LENGTH, MAX_TIMEOUT_MS) == HAL_OK) {
        ParseData(data);
    }
}
...
/*    uint32_t currentTime = HAL_GetTick();
    ...
    switch(GPIO_Pin)
    {
        case GPIO_PIN_5: SetLed(X, ON); break;
        case GPIO_PIN_6: SetLed(Y, ON); break;
        case GPIO_PIN_7: SetLed(X, OFF); break;
        case GPIO_PIN_8: SetLed(Y, OFF); break;
        case GPIO_PIN_9:
        {
            SetLed(ALL, GET);
